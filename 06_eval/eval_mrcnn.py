...
def get_mrcnn_data(fp_id):
  segmentation = np.load(GA_ROOT + 'mrcnn_results/all/%s_pred.npy' % fp_id)
...
semantic_fs = glob.glob(GA_ROOT + 'mrcnn_results/all/*_pred.npy')
...
for idx, fp_id in enumerate(fp_ids):
  ...
  pred_data = get_mrcnn_data(fp_id)
  assert gt_data['segmentation'].shape == pred_data['segmentation'].shape
...
with open('mrcnn_gt.csv', 'w') as f:
  for fp_id, gt_id, conv_id in all_gt_mappings:
    f.write('%s,%d,%d\n' % (fp_id, gt_id, conv_id))

with open('mrcnn_pred.csv', 'w') as f:
  for fp_id, conv_id, gt_id in all_pred_mappings:
    f.write('%s,%d,%d\n' % (fp_id, conv_id, gt_id))

with open('mrcnn.txt', 'w') as f:
  # corner
